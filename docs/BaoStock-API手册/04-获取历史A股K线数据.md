...
示例数据：[下载](assets/history_A_stock_k_data.xlsx)
...
- fields：指示简称，支持多指标输入，以半角逗号分隔，填写内容作为返回类型的列。**详细指标列表见[历史行情指标参数章节](04-获取历史A股K线数据.md#历史行情指标参数)，日线与分钟线参数不同**。此参数不可为空；
- start：开始日期（包含），格式“YYYY-MM-DD”，为空时取2015-01-01；
- end：结束日期（包含），格式“YYYY-MM-DD”，为空时取最近一个交易日；
- frequency：数据类型，默认为d，日k线；d=日k线、w=周、m=月、5=5分钟、15=15分钟、30=30分钟、60=60分钟k线数据，不区分大小写；指数没有分钟线数据；周线每周最后一个交易日才可以获取，月线每月最后一个交易日才可以获取。
- adjustflag：**复权类型，默认不复权：3；1：后复权；2：前复权。已支持分钟线、日线、周线、月线前后复权。** BaoStock提供的是**涨跌幅复权算法**复权因子，具体介绍见：[BaoStock复权因子简介](assets/BaoStockAdjustFactorIntro.pdf "BaoStock复权因子简介.pdf")。
...
BaoStock使用“涨跌幅复权法”进行复权，详细说明参考上文“[复权因子简介](assets/BaoStockAdjustFactorIntro.pdf)”。不同系统间采用复权方式可能不一致，导致数据不一致。
